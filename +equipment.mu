...
** Modified to use formatcolumns() and formattext() from Byzantine Opal.

================================================================================

Dependencies:
	* formattext() - see Layout Functions
	* formatcolumns() - see Byzantine Opal Layout Functions
	* moniker()
	* wheader()
	* wfooter()
	* alert()
...
@@ %2 - equipment list
&layout.short-equipment [v(d.eqf)]=formatcolumns(iter(%2, strcat(xget(%vD, trim(itext(0), l, _)), setq(0, ulocal(f.get-equipment-note, %1, itext(0))), if(t(%q0), strcat(%b, %(, %q0, %)))), if(strmatch(%2, *|*), |), |), |, %0)

@@ %0 - player
@@ %1 - tag if there is one
&layout.list-equipment [v(d.eqf)]=strcat(wheader(if(t(%1), Equipment tagged '%1', Equipment tags), %0), %R, if(t(%1), ulocal(layout.short-equipment, %0,, ulocal(f.sort-equipment-attrs, ulocal(f.list-equipment-by-tag, %1), %b)), formatcolumns(ulocal(f.list-equipment-tags, %1), |, %0)), %R, wfooter(, %0))
...
&layout.equipment-details [v(d.eqf)]=if(t(setr(0, first(ulocal(f.find-all-equipment-by-title, %1), |))), strcat(wheader(ulocal(f.get-equipment-title, %q0), %0), %R, formattext(ulocal(f.get-equipment-details, %q0),, %0), %R, formattext(strcat(%r, Tags:, %b, setq(1, ulocal(f.get-equipment-tags, %q0)), if(t(%q1), itemize(%q1, |), None.)),, %0), %R, if(t(%2), strcat(if(hasattr(%2, _%q0), formattext(rest(xget(%2, _%q0), |),, %0)), if(hasattr(%2, edit(_%q0, NAME, note)), strcat(%r, formattext(strcat(%r, Note:, %b, xget(%2, edit(_%q0, NAME, note))),, %0),%r)))), wfooter(, %0)), ulocal(layout.error, Could not find equipment '%1'.))
